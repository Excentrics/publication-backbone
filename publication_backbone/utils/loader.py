...
CLASS_PATH_ERROR = '''django-publication-backbone is unable to interpret settings value for %s. %s should ' \
...
def get_model_string(model_name, setting_format='PUBLICATION_%s_MODEL', fallback_format='publication_backbone.%s'):
    """
...
def get_publication_backbone_model_string(model_name):
    return get_model_string(model_name, setting_format='PUBLICATION_BACKBONE_%s_MODEL', fallback_format='publication_backbone.%s')